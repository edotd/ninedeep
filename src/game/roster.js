...
import { ageMultiplier } from './aging';
...
// player's current age multiplier, same as cardTotal — an aging roster's actual dice
// modifiers shrink even though the printed card stats never change.
export function offenseStatSum(team, idsOverride) {
  ...
  const sum = ids.reduce((s, id) => {
    ...
    return s + (c.stats.SCO + c.stats.PLM) * ageMultiplier(c.age, c.extendedPrime);
  }, 0);
...
export function defenseStatSum(team, idsOverride) {
  ...
  const sum = ids.reduce((s, id) => {
    const c = team.hand.find((h) => h.id === id);
    return s + (c.stats.DEF + c.stats.REB) * ageMultiplier(c.age, c.extendedPrime);
  }, 0);
  return Math.round(sum);
}
