...
export default function ContractsScreen({ state, actions, myTeamId }) {
  ...
  return <OffseasonFile state={state} team={team}>
    ...
    {expired.length > 0 && <><div className="of-section-label">EXPIRING · RENEW OR RELEASE</div>{expired.map((c) => <div className="of-table-row expired" key={c.id}><span>{c.archetype}<small>{c.position}</small></span><span>{formatCoins(offseasonPrice(team, c.salary))}</span><button className="secondary" disabled={filed || team.hand.length >= 9} onClick={() => actions.renewExpiredContract(myTeamId, c.id)}>RENEW</button></div>)}</>}
