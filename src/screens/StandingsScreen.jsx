          ...
          {showBreakdown && (
            <section className="season-breakdown" aria-label="Season breakdown">
              <div className="season-breakdown-intro">
                ...
                <p>Your base rating comes from the active five, coach and continuity bonuses, Synergy, bench output, active output Gameplans, and any incomplete-roster penalty. Every team then receives a season roll from −2.5% to +2.5%. Seeding Gameplans apply last. The highest final rating earns the top seed.</p>
              </div>
              <div className="season-breakdown-table">
                <div className="season-breakdown-row head">
                  <span>Team</span><span>Bench</span><span>Roster</span><span>Base</span><span>Season Roll</span><span>Gameplan</span><span>Final</span>
                </div>
                {breakdown.map((row) => {
                  ...
                  return (
                    <div key={row.teamId} className={'season-breakdown-row' + (row.teamId === myTeamId ? ' you' : '')}>
                      ...
                      <span>{row.benchRating ? `+${row.benchRating}` : '—'}</span>
