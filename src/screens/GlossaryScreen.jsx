    ...
    <>
      <div className="screen">
        ...
        {SKILLSETS.map((skill) => <div key={skill.id} className="matchup-box">
          ...
          <ul>
            {SKILLSET_PAIRS.filter((p) => p.skills.includes(skill.id)).map((p) => <li key={p.skills.join(':')}>
              {SKILLSETS.find((s) => s.id === p.skills.find((id) => id !== skill.id)).name} — {p.name} · +{p.percent}% {p.side}
            </li>)}
            {POSITION_SKILLSET_BONUSES.filter((b) => b.skillsetId === skill.id).map((b) => <li key={b.name}>
              Starting at {b.position} (solo, no partner needed) — {b.name} · +{b.percent}% {b.side}
            </li>)}
          </ul>
        </div>)}
