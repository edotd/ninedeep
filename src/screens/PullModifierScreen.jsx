...
// Adjustment cards are now dealt automatically the moment the player advances from the Hand
    ...
    <>
      <div className="screen deal-screen">
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
          <h1>Adjustment Cards — Season {state.season}</h1>
          {dealing && <button className="reset-link" style={{ flexShrink: 0, marginLeft: 10 }} onClick={handleSkip}>Skip ▸▸</button>}
        </div>
        <p className="lede">Your {cards.length} Adjustment cards for the season. They stay with you the whole season, can't be traded or returned, and a fresh set is dealt next season.</p>
